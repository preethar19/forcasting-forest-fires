...
Our first step of our exploratory data analysis is to look at the shape of the
distributions of each variable. This gives us a sense of which 
...
Next, using backwards selection from main_fire_model, we construct
...
$$log-odds(\hat{Target})  = -6.727 + 157.331ETo - 0.070solrad + 0.434avgsoiltemp + 5.255windrun + 0.002(dewpoint^2) - 0.374avgairtemp - 127.386avgwindspeed$$
...
$$log-odds(\hat{Target})  = -6.727 + 157.331(ETo) - 0.070(solrad) + 0.434(avgsoiltemp) + 5.255(windrun) + 0.002(dewpoint^2) - 0.374(avgairtemp) - 127.386(avgwindspeed)$$
...
can identify the conditions, not much can be done to alter these conditions
