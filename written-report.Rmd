...
(citation). Thus, in our project, we are trying to determine what are the 
...
```
It's clear that our sampled data includes significantly more "No Fire" 
...
This means that, holding all other variables constant, for every one unit increase
in ETo, the odds that a fire occurs increases by 2.128 (exp(157.331)). Furthermore
for every one unit increase in solar radiation, holding all else constant the 
odds that a fire occurs increases by 0.932 (exp(-0.070)). Holding all other 
variables constant, for every one unit increase in the average soil temperature, 
the odds that a fire occurs increases by 1.543 (exp(0.434)). Additionally, 
holding all other variables constant, a one unit increase in windrun increases 
the odds a fire occurs by 191.521 (exp(5.255)). For everyone one unit squared 
increase in dewpoint, holding all other variables constant, increases the odds 
of a fire by 1.002 (exp(0.002)). Holding all other variables constant, a one unit 
increase in average air temperature increases the odds of a fire by 0.688 
(exp(-0.374)). Finally, holding all other variables constant, a one unit increase 
in average wind speed increases the odds of a fire by 4.752949e-56 (exp(-127.386)).
