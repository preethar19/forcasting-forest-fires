...
(("average average" for interpret)) ((drop-in deviance to see what variables are significant))
((ROC curve to find threshold for prediciton "final model"))
```{r}
new_fire_model <- step(fire_model, direction = "backward")
tidy(new_fire_model, conf.int = TRUE)
```
((This model only has two predictors, so we transformed some variables to see if that would give further insight. quadratic trans formation for wind speed and humidity, by looking at empirical log plot and observing shape))
...
```{r}
fire_model2 <- glm(Target ~ ETo + solrad + avgvappress + avgsoiltemp + windrun + dewpoint + avgairtemp + I(avgwindspeed^2)  + I(avgrelhum^2), data = newestfire, 
                  family = "binomial")
tidy(fire_model2)
```
...
```{r}
fire_model4 <- glm(Target ~ ETo + solrad * avgsoiltemp + avgvappress + avgairtemp + avgwindspeed + I(avgwindspeed^2) * windrun + avgrelhum + I(avgrelhum^2) * dewpoint, data = newestfire, 
                  family = "binomial")
tidy(fire_model4)
```
```{r}
...
```{r}
fire_model5 <- step(fire_model4, direction = "backward")
tidy(fire_model5, conf.int = TRUE)
```
```{r}
glance(fire_model5)
```
