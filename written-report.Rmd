...
climate change will not manifest in a single event, rather in a series of natural 
...
(Louie, 2020). Thus, in our project, we are trying to determine what are the 
...
Firstly, in the short run, we will be able to identify the conditions that make 
forest fires more likely. Thus, we can try to correct these conditions in the 
long run and see if it is possible to mitigate the threat of forest fires in the
future. If we know what predictors play a role in predicting fires, it can help 
authorities better understand what days or seasons present a higher risk, and 
thus prepare accordingly. While climate change will continue to affect our way 
of life, insights into how to manage its immediate consequences and effects will 
help us plan for both the short term and long term future, at least until policy 
and research catch up to the severity of the issue.
...
environmental condition feature.” This user’s data contains a mixture of 
...
The response variable we are investigating is Target, which corresponds to 
...
First, we group observations by station id number. Next, one day is
chosen at random from each of the stations. This gives us 153 observations 
...
In addition, taking a smaller, random sample of the data allows us to ensure 
that randomness is satisfied for our analysis. 

Furthermore, taking a random sample ensures that the data satisfies 
independence, because whether a fire is reported or not is no longer conditional
on surrounding stations and surrounding environmental variables, since each 
...
Even after reducing our dataset to 153 random observations, it is clear that
there are still some stations with missing observations in some variable 
categories. We have decided to use only complete observations in the analysis, 
and thus, the total number of observations is further reduced to 143.

Glimpsing the data, it does not appear that the observations with missingness 
differ systematically from the complete observations; it is thus unlikely that 
our resulting analysis is biased by the decision to remove the data. It appeared
as though the missingness was spread somewhat randomly, and not reflective of 
critical values or possible outliers. Thus, these observations removal will not
systematically impact our findings.
...
Our first step of our exploratory data analysis is to look at the distribution
...
observations as opposed to "Fire" observations. 

The next step is to look at the shape of the distributions of each potential 
predictor variable. This gives us a better understanding of our data and hints 
...
After identifying our regression method, we begin to construct prediction 
models. We begin first with a main effects model containing all possible 
predictors.
...
we suspect multicollinearity among variables. We use vif to further 
...
We remove the variable representing the sum of wind speed over the month 
...
dewpoint are removed as well due to multicollinarity with average vapor 
pressure and average air temperature, respectively. We then construct a new 
model,**full_fire_model**, without the aforementioned variable.
...
Next, using backwards selection from **full_fire_model**, we construct
**reduced_fire_model**. This was done to remove unnecessary variables and thus improve 
...
We then try out variable transformations to potentially bolster the predictive 
power of our model.
...
```{r}
fire_model_aug <- augment(fire_model)

ggplot(data = fire_model_aug, aes(x = dewpoint, y = .resid)) +
  geom_point() + 
  geom_hline(yintercept = 0, color = "red") +
  labs(x = "dewpoint", 
       y = "Residual", 
       title = "Residuals vs. dewpoint")
```
**************************

To determine if this might be the case, we test a quadratic transformation of 
...
Next, we explore potentially significant interaction terms. We ultimately choose 
to test the only interaction term which seemed meaningful: ETo*avgwindspeed. We 
infer that large amounts of water transferred to the land by means of plants 
(ETo) and high wind speed together would significantly decrease the log 
odds of a forest fire happening because a lot of fast-moving wind may spread the 
water more and thus make it harder for a fire to develop in the area. On the 
other end of the spectrum, the log odds of a forest fire happening probably
increases in regions with less plant based transpiration and low average wind
speed because there is much less spreading of moisture in the air.

To determine if this interaction term is statistically significant, we add it
to the model with the quadratic dewpoint term (shown above) and conduct a 
...
Thus, we do not include the interaction term in our final model.
...
Holding all other variables constant, for every one unit increase
in ETo, the odds that a fire occurs increases by 2.128 (exp(157.331)). 

Holding all other variables constant, for every one unit increase in solar 
radiation, the odds that a fire occurs increases by 0.932 (exp(-0.070)). 
...
Holding all other variables constant, a one unit increase in windrun increases 
the odds a fire occurs by 191.521 (exp(5.255)). 

Holding all other variables constant, every one unit squared increase in dewpoint 
increases the odds of a fire by 1.002 (exp(0.002)). 

Holding all other variables constant, a one unit increase in average air 
temperature increases the odds of a fire by 0.688 (exp(-0.374)). 

Holding all other variables constant, a one unit increase in average wind speed 
increases the odds of a fire by 4.752949e-56 (exp(-127.386)).
...
To check linearity, we calculate an empirical logistic regression plot for each 
...
We check randomness based on the context of the data and how the observations 
...
To test our model, we first construct an ROC curve to identify a prediction 
threshold.
...
From this ROC curve, we select a prediction threshold of 0.193, identified by 
minimizing 1-specificity while maximizing sensitivity. Because there is greater
risk in failing to predict a fire (type 2 error), we are less interested in the
...
To truly test our model, we randomly select a new set of observations from 
our original dataset and assess the model's predictive power on the new 
data points.
...
forest fire. Being able to identify these conditions can help make the
proper authorities aware of the possibility that a fire is likely. 
However, even once they are aware of the conditions for forest fires as 
suggested by our model, it is probably too late to be able to do anything 
to effectively prevent the fire at that point, and firefighting measures 
can only be reactive from there.
